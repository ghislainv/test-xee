...
def make_grid(extent, buff, csize, scale, proj=4326,
              ofile="grid.gpkg"):
    ...
    :param csize: Size of the grid cells (same unit as extent).
    ...
    xmax = extent[2] + buff
    ymax = extent[3] + buff
    # Adapt csize to scale
    csize = int(np.round(csize / scale)) * scale
    # List of x coordinates
    xlist = list(np.arange(xmin, xmax + csize, csize))
    nx = len(xlist)
    # List of y coordinates
    ylist = list(np.arange(ymin, ymax + csize, csize))
    ny = len(ylist)
...
def grid_intersection(grid, input_grid, output_grid, borders_shp):
    """Grid intersection.

    :param grid: List of extents for grid cells.
    :param input_grid: Inpugt grid vector file (`.gpkg`).
    :param output_grid: Output grid vector file (`.gpkg`).
    :param borders_shp: Border vector file (`.shp`).
    ...
    lay_g = ds_g.GetLayer()
    # Borders
    dr_b = ogr.GetDriverByName("ESRI Shapefile")
    ds_b = dr_b.Open(borders_shp)
    lay_b = ds_b.GetLayer()
    # New grid
    grid_i = []
