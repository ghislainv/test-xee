...
# ee_jrc.run_task
def run_task(iso3, years, extent_latlong, scale=0.0002727272727, proj=None):
    """Compute forest-cover change with Google Earth Engine.

    Compute the forest-cover change from Joint Research Center (JRC)
    Vancutsem et al. data (v1\\_2020) with Python GEE API and XEE backend.
    ...
    * GEE API Python client is needed: \
    ...
    :param scale: Resolution in degrees per pixel (as the default unit
        of the projection).  Default to 0.0002727272727, which
        corresponds to 30m.
